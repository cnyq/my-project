...
const modulesFiles = require.context(
	"@/components/appConfigurable/configModule",
	true,
	/\b((?!demo).)+\.vue$/
);

const configModule = modulesFiles.keys().reduce((configModule, modulePath) => {
	const moduleName = modulePath.split("/")
		.pop()
		.replace(/\.\w+$/, "");
	const value = modulesFiles(modulePath);
	configModule[moduleName] = value.default;
	return configModule;
}, {});
export {
	configModule
...
const modulesFiles = require.context('../formConstant', true, /\b((?!index).)+\.js$/);

const formConstant = modulesFiles.keys().reduce((formConstant, modulePath) => {
	const moduleName = modulePath.replace(/^\.\/(.*)\.\w+$/, '$1');
	const value = modulesFiles(modulePath)[moduleName] || {};
	formConstant = {
		...formConstant,
		...value
	};
