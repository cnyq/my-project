<template>
    <div class="upload" :class="{
        ...
        <el-upload action ref="upload" :class="{
                'disabled-upload': disabled === '' || disabled
            }" :disabled="disabled === '' || disabled" list-type="picture-card" :file-list="waitingUploadList"
            :accept="accept" :limit="limit" :multiple="multiple" :on-exceed="handleExceed"
            :before-upload="beforeUpload">
            <i class="el-icon-plus"></i>
            <div slot="file" slot-scope="{file}" class="el-upload-list__slot">
                <el-progress v-show="file.uploadding" type="circle" :percentage="uploadPercentage" :width="100-0">
                </el-progress>
                <template v-show="!file.uploadding">
                    <img class="el-upload-list__item-thumbnail" :src="file.fileThumbnail" alt />
        ...
        <viewer v-show="false" :images="waitingUploadListPreview">
            <img v-for="(src,index) in waitingUploadListPreview" width="100" :ref="'prevFile'+ index" :src="src"
                :key="index" />
        </viewer>
        <el-dialog class="upload-fullscreen-dialog" :title="previewTitle" :visible.sync="previewVisible" fullscreen>
...
export default {
            ...
            handler(newList) {
                ...
                newList.forEach(item => {
                    if (item.fileType === 'image') {
                        waitingUploadListPreview.push(
                            item.fileUrl || item.fileThumbnail
                        )
                    }
                })
    ...
    methods: {
            ...
            if (file.fileType === 'image') {
                ...
                image.onload = function() {
                    ...
                    if (window.navigator.msSaveOrOpenBlob) {
                        ...
                        link.setAttribute(
                            'href',
                            window.URL.createObjectURL(url.src)
                        )
        ...
        handleRemove(file) {
            this.waitingUploadList.splice(
                this.waitingUploadList.indexOf(file),
                1
            )
        ...
        beforeUpload(file) {
            ...
            if (this.size && file.size > this.size) {
                this.$message.warning(
                    `文件大小不能超过 ${calcFileSizeInfo(this.size)}`
                )
                return false
            }
        ...
        singleUpload(fileItem) {
            ...
            this.$store.dispatch('qiniu/getQiniuToken').then(token => {
                this.uploadQiniu(token, fileItem).then(fileUrl => {
                    ...
                    const loaddingItemIndex = this.waitingUploadList.findIndex(
                        item => {
            ...
            return new Promise((resolve, reject) => {
                ...
                if (file) {
                    ...
                    observable.subscribe(
                        function(res) {
                            that.uploadPercentage = Math.floor(
                                res.total.percent
                            )
                        },
                        function() {},
